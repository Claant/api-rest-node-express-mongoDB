...
export const login = async (req, res) => {
  try {
    ...
    if (!respuestaPassword)
      return res.status(400).json({ error: "Contraseña incorrecta" }); // si respuestPassword es false, es decir, si las contraseñas no coinciden, se responde con un error 400 y un mensaje de error indicando que la contraseña es incorrecta.
...
export const refreshToken = (req, res) => {
  try {
    const { token, expiresIn } = generateToken(req.uid); // aca se llama a la funcion generateToken que se definio en el archivo utils/generateToken.js, y se le pasa el id del usuario que se obtuvo del token de refresco, y se le asigna a la variable token, esta variable va a ser un objeto que contiene el token generado y el tiempo de expiracion del token.
